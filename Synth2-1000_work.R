...
for (i in 1:length(donn_list)) {
  ...
  for (j in 1:20 ){
    ...
    param <- log.app(Xapp, zapp, 1, 0.00001);
    
    #applying values on individuals of sets 
    train_set <- log.val(param$beta, Xapp)$pred;
    test_set <- log.val(param$beta, Xtst)$pred;
...
#/!\-----Quadratic logisitic regression------/!\
#-------------------------------------
